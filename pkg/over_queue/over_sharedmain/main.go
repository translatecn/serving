...
import (
	...
	"go.uber.org/zap"
	"knative.dev/serving/pkg/over_queue/certificate"
...
const (
	...
	certPath = over_queue.CertDirectory + "/" + over_certificates.CertName

	// keyPath is the path for the server certificate key mounted by queue-proxy.
	keyPath = over_queue.CertDirectory + "/" + over_certificates.PrivateKeyName
)
...
func Main(opts ...Option) error {
	...
	if env.TracingConfigBackend != tracingconfig.None {
		oct := over_tracing.NewOpenCensusTracer(over_tracing.WithExporterFull(env.ServingPod, env.ServingPodIP, logger))
	...
	protoStatReporter := over_queue.NewProtobufStatsReporter(env.ServingPod, reportingPeriod)
...
func buildProbe(logger *zap.SugaredLogger, encodedProbe string, autodetectHTTP2 bool, multiContainerProbes bool) *over_readiness.Probe {
	coreProbes, err := over_readiness.DecodeProbes(encodedProbe, multiContainerProbes)
	if err != nil {
		logger.Fatalw("Queue container failed to parse readiness probe", zap.Error(err))
	}
	if autodetectHTTP2 {
		return over_readiness.NewProbeWithHTTP2AutoDetection(coreProbes)
	}
	return over_readiness.NewProbe(coreProbes)
}

func buildBreaker(logger *zap.SugaredLogger, env config) *over_queue.Breaker {
	...
	params := over_queue.BreakerParams{
	...
	return over_queue.NewBreaker(params)
}
...
func requestAppMetricsHandler(logger *zap.SugaredLogger, currentHandler http.Handler, breaker *over_queue.Breaker, env config) http.Handler {
	h, err := over_queue.NewAppRequestMetricsHandler(
		currentHandler,
		breaker,
...
func requestMetricsHandler(logger *zap.SugaredLogger, currentHandler http.Handler, env config) http.Handler {
	h, err := over_queue.NewRequestMetricsHandler(currentHandler, env.ServingNamespace,
