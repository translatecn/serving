...
import (
	...
	"knative.dev/serving/pkg/over_queue/health"
...
func mainHandler(
	...
	composedHandler = over_queue.ProxyHandler(breaker, stats, tracingEnabled, composedHandler) // 计数 ...
	composedHandler = over_queue.ForwardedShimHandler(composedHandler)                         // ✅
	composedHandler = over_handler.NewTimeoutHandler(composedHandler, "request timeout", func(r *http.Request) (time.Duration, time.Duration, time.Duration) {
	...
	if tracingEnabled {
		composedHandler = over_tracing.HTTPSpanMiddleware(composedHandler)
	}
...
func adminHandler(ctx context.Context, logger *zap.SugaredLogger, drainer *pkghandler.Drainer) http.Handler {
	...
	mux.HandleFunc(over_queue.RequestQueueDrainPath, func(w http.ResponseWriter, r *http.Request) {
