...
import (
	...
	"knative.dev/serving/pkg/over_network/over_handlers"
	...
	if opts.SecretName != "" {
		...
		webhook.tlsConfig = &tls.Config{
			...
			GetCertificate: func(*tls.ClientHelloInfo) (*tls.Certificate, error) {
				secret, err := secretInformer.Lister().Secrets(over_system.Namespace()).Get(opts.SecretName)
				if err != nil {
...
func (wh *Webhook) Run(stop <-chan struct{}) error {
	logger := wh.Logger
	ctx := over_logging.WithLogger(context.Background(), logger)

	drainer := &over_handlers.Drainer{
		Inner:       wh,
