...
func NewCallback(function func(context.Context, *unstructured.Unstructured) error, supportedVerbs ...over_webhook.Operation) Callback {
	m := make(map[over_webhook.Operation]struct{})
...
var _ over_webhook.AdmissionController = (*reconciler)(nil)
...
func (ac *reconciler) Admit(ctx context.Context, request *admissionv1.AdmissionRequest) (resp *admissionv1.AdmissionResponse) {
	...
	if err != nil {
		return over_webhook.MakeErrorStatus("decoding request failed: %v", err)
	}
	...
	if errors != nil {
		return over_webhook.MakeErrorStatus("validation failed: %v", errors)
	}

	if err := ac.callback(ctx, request, gvk); err != nil {
		return over_webhook.MakeErrorStatus("validation callback failed: %v", err)
	}
...
func (ac *reconciler) decodeRequestAndPrepareContext(
	...
	gvk schema.GroupVersionKind,
) (context.Context, over_resourcesemantics.GenericCRD, error) {
	...
	var newObj over_resourcesemantics.GenericCRD
	if len(newBytes) != 0 {
	...
	var oldObj over_resourcesemantics.GenericCRD
	if len(oldBytes) != 0 {
...
//nolint:stylecheck
func validate(ctx context.Context, resource over_resourcesemantics.GenericCRD, req *admissionv1.AdmissionRequest) (err error, warn []error) {
	logger := over_logging.FromContext(ctx)
