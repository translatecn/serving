...
func (b *standardBuilder) buildElector(ctx context.Context, la reconciler.LeaderAware,
	...
	for _, bkt := range bkts {
		rl, err := resourcelock.New(knativeResourceLock,
			oversystem.Namespace(), // use namespace we are running in
			bkt.Name(),
			b.kc.CoreV1(),
			b.kc.CoordinationV1(),
...
func newStandardBuckets(queueName string, cc ComponentConfig) []reconciler.Bucket {
	...
	return overhash.NewBucketSet(names).Buckets()
}
...
func NewStatefulSetBucketAndSet(buckets int) (reconciler.Bucket, *overhash.BucketSet, error) {
	...
	if ssc.StatefulSetID.ordinal >= buckets {
		return nil, nil, fmt.Errorf("ordinal %d is out of range [0, %d)", ssc.StatefulSetID.ordinal, buckets)
	}
	...
	bs := overhash.NewBucketSet(names)
...
func statefulSetPodDNS(ordinal int, ssc *statefulSetConfig) string {
	return fmt.Sprintf("%s://%s-%d.%s.%s.svc.%s:%s", ssc.Protocol,
		...
		oversystem.Namespace(), overnetwork.GetClusterDomainName(), ssc.Port)
}
