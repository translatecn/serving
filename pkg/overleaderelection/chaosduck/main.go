...
import (
	...
	"knative.dev/serving/pkg/overkflag"
...
var (
	disabledComponents      overkflag.StringSet
	disabledComponentsRegex overkflag.StringSet
...
func buildComponents(ctx context.Context, kc kubernetes.Interface) (components, error) {
	leases, err := kc.CoordinationV1().Leases(oversystem.Namespace()).List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
...
func quack(ctx context.Context, kc kubernetes.Interface, component string, leaders sets.Set[string]) error {
	...
	return kc.CoreV1().Pods(oversystem.Namespace()).Delete(ctx, tribute, metav1.DeleteOptions{})
}
...
func main() {
	ctx, _ := injection.EnableInjectionOrDie(oversignals.NewContext(), nil)
