...
import (
	...
	"go.uber.org/zap"
	"knative.dev/serving/pkg/overqueue/certificate"
	...
	"knative.dev/serving/pkg/overqueue/overreadiness"
...
const (
	...
	certPath = overqueue.CertDirectory + "/" + overcertificates.CertName

	// keyPath is the path for the server certificate key mounted by queue-proxy.
	keyPath = overqueue.CertDirectory + "/" + overcertificates.PrivateKeyName
)
...
func Main(opts ...Option) error {
	...
	if env.TracingConfigBackend != tracingconfig.None {
		oct := overtracing.NewOpenCensusTracer(overtracing.WithExporterFull(env.ServingPod, env.ServingPodIP, logger))
	...
	protoStatReporter := overqueue.NewProtobufStatsReporter(env.ServingPod, reportingPeriod)
	...
	httpServers := map[string]*http.Server{
		...
		"admin":   adminServer(":"+strconv.Itoa(overnetworking.QueueAdminPort), adminHandler),
		"metrics": metricsServer(protoStatReporter),
	}

	if env.EnableProfiling {
		httpServers["profile"] = overprofiling.NewServer(overprofiling.NewHandler(logger, true))
	}
	...
	if tlsEnabled {
		...
		tlsServers["admin"] = adminServer(":"+strconv.Itoa(overnetworking.QueueAdminPort), adminHandler)
...
func buildProbe(logger *zap.SugaredLogger, encodedProbe string, autodetectHTTP2 bool, multiContainerProbes bool) *overreadiness.Probe {
	coreProbes, err := overreadiness.DecodeProbes(encodedProbe, multiContainerProbes)
	if err != nil {
		logger.Fatalw("Queue container failed to parse readiness probe", zap.Error(err))
	}
	if autodetectHTTP2 {
		return overreadiness.NewProbeWithHTTP2AutoDetection(coreProbes)
	}
	return overreadiness.NewProbe(coreProbes)
}

func buildBreaker(logger *zap.SugaredLogger, env config) *overqueue.Breaker {
	...
	params := overqueue.BreakerParams{
	...
	return overqueue.NewBreaker(params)
}
func supportsMetrics(ctx context.Context, logger *zap.SugaredLogger, env config) bool {
	...
	if err := setupMetricsExporter(ctx, logger, env.ServingRequestMetricsBackend,
		env.ServingRequestMetricsReportingPeriodSeconds,
		env.MetricsCollectorAddress); err != nil {
...
func setupMetricsExporter(ctx context.Context, logger *zap.SugaredLogger, backend string, reportingPeriod int, collectorAddress string) error {
	...
	ops := metrics.ExporterOptions{
		...
		Component:      "revision",
		PrometheusPort: overnetworking.UserQueueMetricsPort,
...
func requestAppMetricsHandler(logger *zap.SugaredLogger, currentHandler http.Handler, breaker *overqueue.Breaker, env config) http.Handler {
	h, err := overqueue.NewAppRequestMetricsHandler(
		currentHandler,
		breaker,
		env.ServingNamespace,
		env.ServingService,
		env.ServingConfiguration, env.ServingRevision, env.ServingPod,
	)
	if err != nil {
...
func requestMetricsHandler(logger *zap.SugaredLogger, currentHandler http.Handler, env config) http.Handler {
	h, err := overqueue.NewRequestMetricsHandler(currentHandler, env.ServingNamespace,
...
func requestLogHandler(logger *zap.SugaredLogger, currentHandler http.Handler, env config) http.Handler {
	...
	handler, err := pkghttp.NewRequestLogHandler(currentHandler, pkglogging.NewSyncFileWriter(os.Stdout), env.ServingRequestLogTemplate,
