...
// overnetwork.DefaultDrainTimeout).
type Drainer struct {
...
func (d *Drainer) isHealthCheckRequest(r *http.Request) bool {
	if overnetwork.IsKubeletProbe(r) {
		return true
	}

	for _, ua := range d.HealthCheckUAPrefixes {
		if strings.HasPrefix(r.Header.Get(overnetwork.UserAgentKey), ua) {
			return true
		}
	}
...
func isKProbe(r *http.Request) bool {
	return r.Header.Get(overnetwork.ProbeHeaderName) == overnetwork.ProbeHeaderValue
}

// serveKProbe serve KProbe requests.
func serveKProbe(w http.ResponseWriter, r *http.Request) {
	hh := r.Header.Get(overnetwork.HashHeaderName)
	if hh == "" {
		http.Error(w,
			fmt.Sprintf("a probe request must contain a non-empty %q header", overnetwork.HashHeaderName),
			http.StatusBadRequest)
		return
	}
	w.Header().Set(overnetwork.HashHeaderName, hh)
	w.WriteHeader(http.StatusOK)
}
...
func (d *Drainer) Drain() {
	...
	ch := func() chan struct{} {
		...
		if d.QuietPeriod <= 0 {
			d.QuietPeriod = overnetwork.DefaultDrainTimeout
		}
