...
func process(data []byte) ([]byte, error) {
	...
	example := traverse(content, "data", overconfigmap.ExampleKey)
	if example == nil {
		return nil, nil
	}
	...
	checksum := overconfigmap.Checksum(example.Value)
	existingAnnotation := value(annotations, overconfigmap.ExampleChecksumAnnotation)
	if existingAnnotation != nil {
		...
		annotations.Content = append(annotations.Content,
			strNode(overconfigmap.ExampleChecksumAnnotation), sumNode)
	}
