...
import (
	...
	kubeinformerfactory "knative.dev/serving/pkg/injection/clients/namespacedkube/informers/factory"
	"knative.dev/serving/pkg/overnetwork/overhandlers"
...
type Options struct {
	...
	ControllerOptions *overcontroller.ControllerOptions
	...
	if opts.SecretName != "" {
		...
		webhook.tlsConfig = &tls.Config{
			...
			GetCertificate: func(*tls.ClientHelloInfo) (*tls.Certificate, error) {
				secret, err := secretInformer.Lister().Secrets(oversystem.Namespace()).Get(opts.SecretName)
				if err != nil {
