...
func NewController(
	...
	c := overcontroller.NewContext(ctx,
		wh,
		overcontroller.ControllerOptions{
			WorkQueueName: queueName,
			Logger:        overlogging.FromContext(ctx).Named(queueName),
		})

	// Reconcile when the cert bundle changes.
	secretInformer.Informer().AddEventHandler(cache.FilteringResourceEventHandler{
		FilterFunc: overcontroller.FilterWithNameAndNamespace(key.Namespace, key.Name),
