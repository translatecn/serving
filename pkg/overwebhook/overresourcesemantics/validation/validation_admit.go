...
import (
	...
	"knative.dev/serving/pkg/overwebhook/json"
...
type Callback struct {
	...
	supportedVerbs map[overwebhook.Operation]struct{}
}

// NewCallback creates a new callback function to be invoked on supported verbs.
func NewCallback(function func(context.Context, *unstructured.Unstructured) error, supportedVerbs ...overwebhook.Operation) Callback {
	m := make(map[overwebhook.Operation]struct{})
...
var _ overwebhook.AdmissionController = (*reconciler)(nil)
...
func (ac *reconciler) Admit(ctx context.Context, request *admissionv1.AdmissionRequest) (resp *admissionv1.AdmissionResponse) {
	...
	if err != nil {
		return overwebhook.MakeErrorStatus("decoding request failed: %v", err)
	}
	...
	if errors != nil {
		return overwebhook.MakeErrorStatus("validation failed: %v", errors)
	}

	if err := ac.callback(ctx, request, gvk); err != nil {
		return overwebhook.MakeErrorStatus("validation callback failed: %v", err)
	}
...
func (ac *reconciler) decodeRequestAndPrepareContext(
	...
	gvk schema.GroupVersionKind,
) (context.Context, overresourcesemantics.GenericCRD, error) {
	...
	var newObj overresourcesemantics.GenericCRD
	if len(newBytes) != 0 {
		newObj = handler.DeepCopyObject().(overresourcesemantics.GenericCRD)
	...
	var oldObj overresourcesemantics.GenericCRD
	if len(oldBytes) != 0 {
		oldObj = handler.DeepCopyObject().(overresourcesemantics.GenericCRD)
...
//nolint:stylecheck
func validate(ctx context.Context, resource overresourcesemantics.GenericCRD, req *admissionv1.AdmissionRequest) (err error, warn []error) {
	logger := overlogging.FromContext(ctx)
