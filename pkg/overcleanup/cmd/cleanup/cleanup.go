...
func main() {
	...
	env := overenvironment.ClientConfig{}
	...
	for _, dep := range []string{"domain-mapping", "domainmapping-webhook", "net-certmanager-controller", "net-certmanager-webhook"} {
		if err = client.AppsV1().Deployments(oversystem.Namespace()).Delete(context.Background(), dep, metav1.DeleteOptions{}); err != nil && !apierrs.IsNotFound(err) {
			logger.Fatal("failed to delete deployment ", dep, ": ", err)
		}
	}

	leases, err := client.CoordinationV1().Leases(oversystem.Namespace()).List(context.Background(), metav1.ListOptions{})
	if err != nil {
	...
	for _, lease := range leases.Items {
		if strings.HasPrefix(lease.Name, "domainmapping") ||
			...
			if err = client.CoordinationV1().Leases(oversystem.Namespace()).Delete(context.Background(), lease.Name, metav1.DeleteOptions{}); err != nil && !apierrs.IsNotFound(err) {
	...
	if err = client.CoreV1().Services(oversystem.Namespace()).Delete(context.Background(), "domainmapping-webhook", metav1.DeleteOptions{}); err != nil && !apierrs.IsNotFound(err) {
		logger.Fatal("failed to delete service domainmapping-webhook: ", err)
	}
	if err = client.CoreV1().Secrets(oversystem.Namespace()).Delete(context.Background(), "domainmapping-webhook-certs", metav1.DeleteOptions{}); err != nil && !apierrs.IsNotFound(err) {
	...
	if err = client.CoreV1().Services(oversystem.Namespace()).Delete(context.Background(), "net-certmanager-controller", metav1.DeleteOptions{}); err != nil && !apierrs.IsNotFound(err) {
		logger.Fatal("failed to delete service net-certmanager-controller: ", err)
	}
	if err = client.CoreV1().Services(oversystem.Namespace()).Delete(context.Background(), "net-certmanager-webhook", metav1.DeleteOptions{}); err != nil && !apierrs.IsNotFound(err) {
	...
	if err = client.CoreV1().Secrets(oversystem.Namespace()).Delete(context.Background(), "net-certmanager-webhook-certs", metav1.DeleteOptions{}); err != nil && !apierrs.IsNotFound(err) {
...
func setupLogger() *zap.SugaredLogger {
	...
	config, err := overlogging.NewConfigFromMap(nil)
	if err != nil {
		log.Fatal("Failed to create logging config: ", err)
	}

	logger, _ := overlogging.NewLoggerFromConfig(config, component)
	return logger
}
