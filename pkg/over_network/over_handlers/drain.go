...
// over_network.DefaultDrainTimeout).
type Drainer struct {
...
func (d *Drainer) isHealthCheckRequest(r *http.Request) bool {
	if over_network.IsKubeletProbe(r) {
		return true
	}

	for _, ua := range d.HealthCheckUAPrefixes {
		if strings.HasPrefix(r.Header.Get(over_network.UserAgentKey), ua) {
			return true
		}
	}
...
func isKProbe(r *http.Request) bool {
	return r.Header.Get(over_network.ProbeHeaderName) == over_network.ProbeHeaderValue
}

// serveKProbe serve KProbe requests.
func serveKProbe(w http.ResponseWriter, r *http.Request) {
	hh := r.Header.Get(over_network.HashHeaderName)
	if hh == "" {
		http.Error(w,
			fmt.Sprintf("a probe request must contain a non-empty %q header", over_network.HashHeaderName),
			http.StatusBadRequest)
		return
	}
	w.Header().Set(over_network.HashHeaderName, hh)
	w.WriteHeader(http.StatusOK)
}
...
func (d *Drainer) Drain() {
	...
	ch := func() chan struct{} {
		...
		if d.QuietPeriod <= 0 {
			d.QuietPeriod = over_network.DefaultDrainTimeout
		}
