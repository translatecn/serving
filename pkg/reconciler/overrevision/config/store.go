...
import (
	...
	pkgtracing "knative.dev/serving/pkg/overtracing/config"
)
...
// Store is a typed wrapper around overconfigmap.UntypedStore to handle our configmaps.
type Store struct {
...
// WatchConfigs uses the provided overconfigmap.Watcher
// to setup watches for the config names provided in the
// Constructors map
func (s *Store) WatchConfigs(cmw overconfigmap.Watcher) {
...
func (s *Store) Load() *Config {
	...
	if dep, ok := s.UntypedLoad(overdeployment.ConfigName).(*overdeployment.Config); ok {
		cfg.Deployment = dep.DeepCopy()
	}
	if log, ok := s.UntypedLoad(overlogging.ConfigMapName()).(*overlogging.Config); ok {
