...
// Reconciler implements overcontroller.Reconciler for Revision resources.
...
func (c *Reconciler) ReconcileKind(ctx context.Context, rev *v1.Revision) pkgreconciler.Event { // ✅
	...
	logger := overlogging.FromContext(ctx).Desugar()
	...
	if overconfig.FromContext(ctx).Network.SystemInternalTLSEnabled() {
	...
	if !readyBeforeReconcile && readyAfterReconcile {
		...
		overcontroller.GetEventRecorder(ctx).Event(rev, corev1.EventTypeNormal, "RevisionReady", "Revision becomes ready upon all resources being ready")
...
func (c *Reconciler) updateRevisionLoggingURL(ctx context.Context, rev *v1.Revision) {
	config := overconfig.FromContext(ctx)
