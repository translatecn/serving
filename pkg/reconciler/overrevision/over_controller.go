...
import (
	...
	"knative.dev/serving/pkg/overchangeset"
...
func newControllerWithOptions(
	...
	impl := revisionreconciler.NewImpl(ctx, c, func(impl *overcontroller.Impl) overcontroller.Options {
		...
		configStore := overconfig.NewStore(logger.Named("config-store"), resync)
	...
	if rt, err := newResolverTransport(k8sCertPath, digestResolutionWorkers, digestResolutionWorkers); err != nil {
		overlogging.FromContext(ctx).Errorw("Failed to create resolver transport", zap.Error(err))
	} else {
		transport = rt
	}

	userAgent := fmt.Sprintf("knative/%s (serving)", overchangeset.Get())
	...
	revisionInformer.Informer().AddEventHandler(overcontroller.HandleAll(impl.Enqueue))

	deploymentInformer.Informer().AddEventHandler(cache.FilteringResourceEventHandler{
		FilterFunc: overcontroller.FilterController(&v1.Revision{}),
	...
	certificateInformer.Informer().AddEventHandler(overcontroller.HandleAll(
