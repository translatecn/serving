...
import (
	...
	networkingaccessor "knative.dev/serving/pkg/reconciler/overaccessor/networking"
	...
	overconfig "knative.dev/serving/pkg/reconciler/overrevision/config"
	overresources "knative.dev/serving/pkg/reconciler/overrevision/resources"
	resourcenames "knative.dev/serving/pkg/reconciler/overrevision/resources/names"
)
...
func (c *Reconciler) reconcilePA(ctx context.Context, rev *v1.Revision) error {
	...
	tmpl := overresources.MakePA(rev, deployment)
	logger.Debugf("Desired PASpec: %#v", tmpl.Spec)
	if !equality.Semantic.DeepEqual(tmpl.Spec, pa.Spec) {
		diff, _ := overkmp.SafeDiff(tmpl.Spec, pa.Spec) // Can't realistically fail on PASpec.
...
func (c *Reconciler) reconcileQueueProxyCertificate(ctx context.Context, rev *v1.Revision) error {
	ns := rev.Namespace
	logger := overlogging.FromContext(ctx)
	logger.Infof("Reconciling queue-proxy Knative Certificate for system-internal-tls: %s/%s", ns, overnetworking.ServingCertName)

	certClass := overconfig.FromContext(ctx).Network.DefaultCertificateClass
	...
	desiredCert := overresources.MakeQueueProxyCertificate(owningNs, certClass)
	cert, err := networkingaccessor.ReconcileCertificate(ctx, owningNs, desiredCert, c)
	if err != nil {
		return fmt.Errorf("failed to reconcile Knative certificate %s/%s: %w", ns, overnetworking.ServingCertName, err)
	}

	// Verify the secret is created and has been added the certificates
	secret, err := c.kubeclient.CoreV1().Secrets(ns).Get(ctx, overnetworking.ServingCertName, metav1.GetOptions{})
	if apierrs.IsNotFound(err) {
		return fmt.Errorf("secret %s/%s is not ready yet: secret could not be found", ns, overnetworking.ServingCertName)
	} else if err != nil {
		return fmt.Errorf("secret %s/%s is not ready yet: %w", ns, overnetworking.ServingCertName, err)
	}

	if _, ok := secret.Data[overcertificates.CertName]; !ok {
		return fmt.Errorf("certificate in secret %s/%s is not ready yet: public cert not found", ns, overnetworking.ServingCertName)
	}
	if _, ok := secret.Data[overcertificates.PrivateKeyName]; !ok {
		return fmt.Errorf("certificate in secret %s/%s is not ready yet: private key not found", ns, overnetworking.ServingCertName)
	}
...
func (c *Reconciler) reconcileDeployment(ctx context.Context, rev *v1.Revision) error {
	ns := rev.Namespace
	deploymentName := resourcenames.Deployment(rev)
	logger := overlogging.FromContext(ctx).With(zap.String(logkey.Deployment, deploymentName))
	...
	if *deployment.Spec.Replicas > 0 && deployment.Status.AvailableReplicas == 0 {
		...
		if len(pods.Items) > 0 {
			...
			for _, status := range pod.Status.ContainerStatuses {
				if status.Name != overresources.QueueContainerName {
...
func (c *Reconciler) reconcileImageCache(ctx context.Context, rev *v1.Revision) error {
	...
	for _, container := range append(rev.Status.ContainerStatuses, rev.Status.InitContainerStatuses...) {
		imageName := overkmeta.ChildName(resourcenames.ImageCache(rev), "-"+container.Name)
