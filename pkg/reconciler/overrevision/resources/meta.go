...
func makeLabels(revision *v1.Revision) map[string]string {
	labels := overkmeta.FilterMap(revision.GetLabels(), excludeLabels.Has)
	labels = overkmeta.UnionMaps(labels, map[string]string{
...
func makeAnnotations(revision *v1.Revision) map[string]string {
	return overkmeta.FilterMap(revision.GetAnnotations(), excludeAnnotations.Has)
}
