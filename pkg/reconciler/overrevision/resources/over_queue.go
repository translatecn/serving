...
var (
	queueHTTPPort = corev1.ContainerPort{
		...
		ContainerPort: overnetworking.BackendHTTPPort,
	}
	queueHTTP2Port = corev1.ContainerPort{
		Name:          requestQueueHTTPPortName,
		ContainerPort: overnetworking.BackendHTTP2Port,
	}
	queueHTTPSPort = corev1.ContainerPort{
		Name:          requestQueueHTTPSPortName,
		ContainerPort: overnetworking.BackendHTTPSPort,
	}
	queueNonServingPorts = []corev1.ContainerPort{{
		// Provides health checks and lifecycle hooks.
		Name:          v1.QueueAdminPortName,
		ContainerPort: overnetworking.QueueAdminPort,
	}, {
		Name:          v1.AutoscalingQueueMetricsPortName,
		ContainerPort: overnetworking.AutoscalingQueueMetricsPort,
	}, {
		Name:          v1.UserQueueMetricsPortName,
		ContainerPort: overnetworking.UserQueueMetricsPort,
	}}

	profilingPort = corev1.ContainerPort{
		Name:          profilingPortName,
		ContainerPort: overprofiling.ProfilingPort,
	}

	queueSecurityContext = &corev1.SecurityContext{
		AllowPrivilegeEscalation: overptr.Bool(false),
		ReadOnlyRootFilesystem:   overptr.Bool(true),
		RunAsNonRoot:             overptr.Bool(true),
...
func createQueueResources(cfg *overdeployment.Config, annotations map[string]string, userContainer *corev1.Container, useDefaults bool) corev1.ResourceRequirements {
	...
	for _, r := range []struct {
		...
		RequestDefault: &overdeployment.QueueSidecarCPURequestDefault,
		Limit:          cfg.QueueSidecarCPULimit,
		LimitDefault:   &overdeployment.QueueSidecarCPULimitDefault,
	}, {
		Name:           corev1.ResourceMemory,
		Request:        cfg.QueueSidecarMemoryRequest,
		RequestDefault: &overdeployment.QueueSidecarMemoryRequestDefault,
		Limit:          cfg.QueueSidecarMemoryLimit,
		LimitDefault:   &overdeployment.QueueSidecarMemoryLimitDefault,
	}, {
...
func resourceFromAnnotation(m map[string]string, key overkmap.KeyPriority) (resource.Quantity, bool) {
	_, v, _ := key.Get(m)
	q, err := resource.ParseQuantity(v)
	return q, err == nil
}

func fractionFromPercentage(m map[string]string, key overkmap.KeyPriority) (float64, bool) {
	_, v, _ := key.Get(m)
...
func makeQueueContainer(rev *v1.Revision, cfg *overconfig.Config) (*corev1.Container, error) {
	configName := ""
	...
	if multiContainerProbingEnabled && readinessProbes != nil && len(readinessProbes) > 0 {
		readinessProbeJSON, err = overreadiness.EncodeMultipleProbes(readinessProbes)
		if err != nil {
			return nil, fmt.Errorf("failed to serialize multiple readiness probes: %w", err)
		}
	} else if userContainerReadinessProbe != nil {
		readinessProbeJSON, err = overreadiness.EncodeSingleProbe(userContainerReadinessProbe)
		if err != nil {
	...
	c := &corev1.Container{
		...
		Env: []corev1.EnvVar{{
			...
			Name:  oversystem.NamespaceEnvKey,
			Value: oversystem.Namespace(),
		}, {
