...
func (c *Reconciler) createImageCache(ctx context.Context, rev *v1.Revision, containerName, imageDigest string) (*caching.Image, error) {
	image := overresources.MakeImageCache(rev, containerName, imageDigest)
...
func (c *Reconciler) createPA(
	...
	pa := overresources.MakePA(rev, deployment)
...
func (c *Reconciler) checkAndUpdateDeployment(ctx context.Context, rev *v1.Revision, have *appsv1.Deployment) (*appsv1.Deployment, error) {
	...
	cfgs := overconfig.FromContext(ctx)

	deployment, err := overresources.MakeDeployment(rev, cfgs) // ✅
	if err != nil {
	...
	desiredDeployment.Labels = overkmeta.UnionMaps(deployment.Labels, desiredDeployment.Labels)
	...
	diff, err := overkmp.SafeDiff(have.Spec, d.Spec)
	if err != nil {
		return nil, err
	}
