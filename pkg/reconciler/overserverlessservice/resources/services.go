...
func MakePublicService(sks *v1alpha1.ServerlessService) *corev1.Service {
	return &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: overkmeta.UnionMaps(sks.GetLabels(), map[string]string{
				// Add our own special key.
				overnetworking.SKSLabelKey:    sks.Name,
				overnetworking.ServiceTypeKey: string(overnetworking.ServiceTypePublic),
			}),
			Annotations:     overkmeta.CopyMap(sks.GetAnnotations()),
			OwnerReferences: []metav1.OwnerReference{*overkmeta.NewControllerRef(sks)},
		},
...
func makePublicServicePorts(sks *v1alpha1.ServerlessService) []corev1.ServicePort {
	ports := []corev1.ServicePort{{
		...
		TargetPort: intstr.FromInt(overnetworking.BackendHTTPSPort),
	}}
	return ports
}
...
func MakePrivateService(sks *v1alpha1.ServerlessService, selector map[string]string) *corev1.Service {
	return &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      overnames.PrivateService(sks.Name),
			Namespace: sks.Namespace,
			Labels: overkmeta.UnionMaps(sks.GetLabels(), map[string]string{
				// Add our own special key.
				overnetworking.SKSLabelKey:    sks.Name,
				overnetworking.ServiceTypeKey: string(overnetworking.ServiceTypePrivate),
			}),
			Annotations:     overkmeta.CopyMap(sks.GetAnnotations()),
			OwnerReferences: []metav1.OwnerReference{*overkmeta.NewControllerRef(sks)},
		},
		Spec: corev1.ServiceSpec{
			Ports: []corev1.ServicePort{{
				...
				Protocol:   corev1.ProtocolTCP,
				Port:       overnetworking.QueueAdminPort,
				TargetPort: intstr.FromInt(overnetworking.QueueAdminPort),
			}, {
...
func targetPort(sks *v1alpha1.ServerlessService) intstr.IntOrString {
	if sks.Spec.ProtocolType == pkgnet.ProtocolH2C {
		return intstr.FromInt(overnetworking.BackendHTTP2Port)
	}
	return intstr.FromInt(overnetworking.BackendHTTPPort)
}
...
func filterSubsetPorts(targetPort int32, subsets []corev1.EndpointSubset) []corev1.EndpointSubset {
	...
	for i, sss := range subsets {
		...
		for j, p := range sss.Ports {
			switch p.Port {
			case overnetworking.BackendHTTPSPort:
				fallthrough
			case targetPort:
