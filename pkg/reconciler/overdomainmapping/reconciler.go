...
import (
	...
	servingnetworking "knative.dev/serving/pkg/overnetworking"
	...
	"knative.dev/serving/pkg/reconciler/overdomainmapping/overresources"
	routeresources "knative.dev/serving/pkg/reconciler/overroute/resources"
	"knative.dev/serving/pkg/resolver"
)

// Reconciler implements overcontroller.Reconciler for DomainMapping resources.
...
func (r *Reconciler) ReconcileKind(ctx context.Context, dm *v1beta1.DomainMapping) reconciler.Event {
	...
	if err := r.reconcileDomainClaim(ctx, dm); err != nil { // 创建 ClusterDomainClaims
		return err
	}
	...
	targetHost, targetBackendSvc, err := r.resolveRef(ctx, dm) // stock-service-example.default.svc.cluster.local stock-service-example
	if err != nil {
		return err
	}
	...
	desired := overresources.MakeIngress(dm, targetBackendSvc, targetHost, ingressClass, httpOption, tls, acmeChallenges...)
...
func (r *Reconciler) resolveRef(ctx context.Context, dm *v1beta1.DomainMapping) (host, backendSvc string, err error) {
	...
	requiredSuffix := ".svc." + overnetwork.GetClusterDomainName()
...
func (r *Reconciler) tls(ctx context.Context, dm *v1beta1.DomainMapping) ([]netv1alpha1.IngressTLS, []netv1alpha1.HTTP01Challenge, error) {
	...
	desiredCert := overresources.MakeCertificate(dm, certClass(ctx))
...
func (r *Reconciler) createDomainClaim(ctx context.Context, dm *v1beta1.DomainMapping) error {
	...
	_, err := r.netclient.NetworkingV1alpha1().ClusterDomainClaims().Create(ctx, overresources.MakeDomainClaim(dm), metav1.CreateOptions{})
	if err != nil {
