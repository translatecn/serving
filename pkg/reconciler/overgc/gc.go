...
import (
	...
	configns "knative.dev/serving/pkg/reconciler/overgc/config"
)
...
func collect(ctx context.Context, client clientset.Interface, revisionLister listers.RevisionLister, config *v1.Configuration) pkgreconciler.Event {
	...
	if max == overgc.Disabled && cfg.RetainSinceCreateTime == overgc.Disabled && cfg.RetainSinceLastActiveTime == overgc.Disabled {
	...
	if max == overgc.Disabled || nonStaleCount <= max {
		return nil
	}
