...
import (
	...
	configresources "knative.dev/serving/pkg/reconciler/overconfiguration/resources"
	"knative.dev/serving/pkg/reconciler/overservice/resources"
	resourcenames "knative.dev/serving/pkg/reconciler/overservice/resources/names"
)

// Reconciler implements overcontroller.Reconciler for Service resources.
...
func (c *Reconciler) createConfiguration(ctx context.Context, service *v1.Service) (*v1.Configuration, error) {
	return c.client.ServingV1().Configurations(service.Namespace).Create(ctx, resources.MakeConfiguration(service), metav1.CreateOptions{})
}
...
func configSemanticEquals(ctx context.Context, desiredConfig, config *v1.Configuration) (bool, error) {
	...
	specDiff, err := overkmp.SafeDiff(desiredConfig.Spec, config.Spec)
	if err != nil {
...
func routeSemanticEquals(ctx context.Context, desiredRoute, route *v1.Route) (bool, error) {
	logger := overlogging.FromContext(ctx)
	specDiff, err := overkmp.SafeDiff(desiredRoute.Spec, route.Spec)
	if err != nil {
...
func (c *Reconciler) reconcileConfiguration(ctx context.Context, service *v1.Service, config *v1.Configuration) (*v1.Configuration, error) {
	...
	diff.Write("Configurations", config, existing)
...
func (c *Reconciler) createRoute(ctx context.Context, service *v1.Service) (*v1.Route, error) {
	return c.client.ServingV1().Routes(service.Namespace).Create(ctx, resources.MakeRoute(service), metav1.CreateOptions{})
}
func (c *Reconciler) reconcileRoute(ctx context.Context, service *v1.Service, route *v1.Route) (*v1.Route, error) {
	...
	diff.Write("Route", route, existing)
...
func (c *Reconciler) checkRoutesNotReady(config *v1.Configuration, logger *zap.SugaredLogger, route *v1.Route, service *v1.Service) {
	...
	if diff, err := overkmp.SafeDiff(got, want, ignoreFields); err != nil || diff != "" {
...
func (c *Reconciler) ReconcileKind(ctx context.Context, service *v1.Service) pkgreconciler.Event { // ✅
	...
	if err := CheckNameAvailability(config, c.revisionLister); err != nil && !apierrs.IsNotFound(err) {
