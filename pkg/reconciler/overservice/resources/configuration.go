...
import (
	...
	"knative.dev/serving/pkg/reconciler/overservice/resources/names"
)
...
func MakeConfigurationFromExisting(service *v1.Service, existing *v1.Configuration) *v1.Configuration {
	...
	anns := overkmap.ExcludeKeyList(service.GetAnnotations(), exclude)

	routeName := names.Route(service)
	set := overlabeler.GetListAnnValue(existing.Annotations, serving.RoutesAnnotationKey)
	...
	return &v1.Configuration{
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{
				*overkmeta.NewControllerRef(service),
			},
			Labels:      overkmeta.UnionMaps(service.GetLabels(), labels),
			Annotations: anns,
		},
