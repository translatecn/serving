...
import (
	...
	"knative.dev/serving/pkg/reconciler/autoscaling/over_kpa/over_resources"
...
type Reconciler struct {
	*areconciler.Base
	...
	deciders   over_resources.Deciders
	scaler     *scaler
}
...
func activeThreshold(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler) int {
	...
	if !pa.Status.IsScaleTargetInitialized() {
		initialScale := over_resources.GetInitialScale(asConfig, pa)
...
func (c *Reconciler) reconcileDecider(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler) (*scaling.ReversionReplicasByLoad, error) {
	desiredDecider := over_resources.MakeReversionReplicasByLoadDecider(
		pa,
...
func computeStatus(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler, pc podCounts, logger *zap.SugaredLogger) {
	...
	pa.Status.ActualScale = over_ptr.Int32(int32(pc.ready))
	...
	if pc.want == -1 && pa.Status.DesiredScale != nil && *pa.Status.DesiredScale >= 0 {
		...
		//nolint:gosec // bound by 0 < x < max(int32)
		pa.Status.DesiredScale = over_ptr.Int32(int32(pc.want))
	}
