...
type Reconciler struct {
	*areconciler.Base
	...
	deciders   overresources.Deciders
	scaler     *scaler
}
...
func (c *Reconciler) ReconcileKind(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler) pkgreconciler.Event { // ✅
	...
	sksName := anames.SKS(pa.Name) //   ksvc名字-版本
	...
	// 拥有 SKS 及其private-service 是后续所有步骤的前提条件。
	if sks == nil || sks.Status.PrivateServiceName == "" {
		// 在我们能够确定决定者（即激活剂的决定因素）并得出实际的激活剂数量之前，我们先从默认值 2 开始。
		if _, err = c.ReconcileSKS(ctx, pa, nv1alpha1.SKSOperationModeProxy, minActivators); err != nil { // 确保sks 没有发生变化
			return fmt.Errorf("error reconciling SKS: %w", err)
		}
		pa.Status.MarkSKSNotReady(noPrivateServiceName)               // In both cases this is true.
		computeStatus(ctx, pa, podCounts{want: scaleUnknown}, logger) // 完善status 属性
		return nil
	}

	pa.Status.MetricsServiceName = sks.Status.PrivateServiceName
	decider, err := c.reconcileDecider(ctx, pa) // 调谐记录的 podautoscale 属性
	if err != nil {
		return fmt.Errorf("error reconciling ReversionReplicasByLoad: %w", err)
	}
	...
	logger.Infof("PA scale got=%d, want=%d, desiredPods=%d ebc=%d", ready, want, decider.Status.DesiredScale, decider.Status.ExcessBurstCapacity)
...
func activeThreshold(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler) int {
	...
	if !pa.Status.IsScaleTargetInitialized() {
		initialScale := overresources.GetInitialScale(asConfig, pa)
...
func (c *Reconciler) reconcileDecider(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler) (*scaling.ReversionReplicasByLoad, error) {
	desiredDecider := overresources.MakeReversionReplicasByLoadDecider(
		pa,
		config.FromContext(ctx).Autoscaler, // configmap 配置文件
	)
	decider, err := c.deciders.Get(ctx, pa.Namespace, pa.Name)
	if errors.IsNotFound(err) {
		decider, err = c.deciders.Create(ctx, desiredDecider)
		if err != nil {
			return nil, fmt.Errorf("error creating ReversionReplicasByLoad: %w", err)
		}
	} else if err != nil {
		return nil, fmt.Errorf("error fetching ReversionReplicasByLoad: %w", err)
	}
...
func computeStatus(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler, pc podCounts, logger *zap.SugaredLogger) {
	// bound by 0 < x < max(int32)
	pa.Status.ActualScale = overptr.Int32(int32(pc.ready))
	...
	if pc.want == scaleUnknown && pa.Status.DesiredScale != nil && *pa.Status.DesiredScale >= 0 {
		logger.Debugf("Ignoring change of desiredScale from %d to %d", *pa.Status.DesiredScale, pc.want)
	} else {
		//  // bound by 0 < x < max(int32)
		pa.Status.DesiredScale = overptr.Int32(int32(pc.want))
	}
...
func computeNumActivators(readyPods int, decider *scaling.ReversionReplicasByLoad) int32 {
