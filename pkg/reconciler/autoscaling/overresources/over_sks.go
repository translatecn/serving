...
import (
	...
	nv1a1 "knative.dev/serving/networking/pkg/apis/networking/v1alpha1"
	"knative.dev/serving/pkg/apis/autoscaling"
	autoscalingv1alpha1 "knative.dev/serving/pkg/apis/autoscaling/v1alpha1"
	"knative.dev/serving/pkg/overkmeta"
	"knative.dev/serving/pkg/reconciler/autoscaling/overresources/names"
)
...
func MakeSKS(pa *autoscalingv1alpha1.PodAutoscaler, mode nv1a1.ServerlessServiceOperationMode, numActivators int32) *nv1a1.ServerlessService {
	return &nv1a1.ServerlessService{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels:    overkmeta.CopyMap(pa.GetLabels()),
			Annotations: overkmeta.FilterMap(pa.GetAnnotations(), func(s string) bool {
				return s == autoscaling.MetricAnnotationKey
			}),
			OwnerReferences: []metav1.OwnerReference{*overkmeta.NewControllerRef(pa)},
		},
