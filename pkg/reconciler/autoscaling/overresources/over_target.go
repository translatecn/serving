...
// ResolveMetricTarget 接收来自多个位置的缩放指标调节旋钮，并将其解析为供自动缩放器使用的最终值。
// `target` 是缩放指标的目标值，即自动缩放器将力求达到的目标值；
// `total` 是该 Pod 允许的最大可能缩放指标值。
