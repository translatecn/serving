...
func MakeRevision(ctx context.Context, configuration *v1.Configuration, tm time.Time) *v1.Revision {
	...
	if rev.Name == "" {
		rev.Name = overkmeta.ChildName(configuration.Name, fmt.Sprintf("-%05d", configuration.Generation))
	}
	...
	rev.OwnerReferences = append(rev.OwnerReferences, *overkmeta.NewControllerRef(configuration))
