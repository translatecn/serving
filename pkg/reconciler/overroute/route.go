...
import (
	...
	kaccessor "knative.dev/serving/pkg/reconciler/overaccessor"
	networkaccessor "knative.dev/serving/pkg/reconciler/overaccessor/networking"
	"knative.dev/serving/pkg/reconciler/overroute/domains"
	"knative.dev/serving/pkg/reconciler/overroute/overconfig"
	"knative.dev/serving/pkg/reconciler/overroute/resources"
	"knative.dev/serving/pkg/reconciler/overroute/resources/labels"
	resourcenames "knative.dev/serving/pkg/reconciler/overroute/resources/names"
	"knative.dev/serving/pkg/reconciler/overroute/traffic"
	"knative.dev/serving/pkg/reconciler/overroute/visibility"
	"knative.dev/serving/pkg/tracker"
)

// Reconciler implements overcontroller.Reconciler for Route resources.
...
func ingressClassForRoute(ctx context.Context, r *v1.Route) string {
	...
	return overconfig.FromContext(ctx).Network.DefaultIngressClass
}
...
func certClass(ctx context.Context, r *v1.Route) string {
	...
	return overconfig.FromContext(ctx).Network.DefaultCertificateClass
}
...
func (c *Reconciler) ReconcileKind(ctx context.Context, r *v1.Route) pkgreconciler.Event {
	...
	if overconfig.FromContext(ctx).Network.ClusterLocalDomainTLS == netcfg.EncryptionEnabled {
...
func (c *Reconciler) externalDomainTLS(ctx context.Context, host string, r *v1.Route, traffic *traffic.Config) (
	...
	var tls []netv1alpha1.IngressTLS
	...
	routeDomain := overconfig.FromContext(ctx).Domain.LookupDomainForLabels(r.Labels)
	labelSelector := kubelabels.SelectorFromSet(kubelabels.Set{
		networking.WildcardCertDomainLabelKey: routeDomain,
	})

	allWildcardCerts, err := c.certificateLister.Certificates(r.Namespace).List(labelSelector) // 通配符域名
	if err != nil {
		return nil, nil, desiredCerts, err
	}
	...
	for _, desiredCert := range desiredCerts {
		...
		if cert.IsReady() {
			...
			if overconfig.FromContext(ctx).Network.HTTPProtocol == netcfg.HTTPEnabled {
				if dnsNames.Has(host) {
...
func (c *Reconciler) updateRouteStatusURL(ctx context.Context, route *v1.Route, visibility map[string]netv1alpha1.IngressVisibility) error {
	...
	scheme := "http"
	if !isClusterLocal {
		scheme = overconfig.FromContext(ctx).Network.DefaultExternalScheme
	}
...
func (c *Reconciler) configureTraffic(ctx context.Context, r *v1.Route) (*traffic.Config, error) {
	logger := overlogging.FromContext(ctx)
	// 从route配置的configuration 以及 revision 中获取信息，填充到 traffic
	...
	for _, configuration := range t.Configurations {
		if err := c.tracker.TrackReference(objectRef(configuration), r); err != nil { // configuration 的owner 变成r
			return nil, err
		}
	}
...
func externalDomainTLSEnabled(ctx context.Context, r *v1.Route) bool {
	if !overconfig.FromContext(ctx).Network.ExternalDomainTLS {
		return false
	}
	...
	if annotationValue != "" && err != nil {
		...
		logger.Warnf("Invalid annotation value for %q. Value: %q", networking.DisableExternalDomainTLSAnnotationKey, annotationValue)
	}
...
func (c *Reconciler) deleteOrphanedCerts(ctx context.Context, orphanCerts []*netv1alpha1.Certificate) {
	recorder := overcontroller.GetEventRecorder(ctx)
