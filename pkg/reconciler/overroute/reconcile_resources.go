...
func deserializeRollout(ctx context.Context, ro string) *traffic.Rollout {
	...
	if err := json.Unmarshal([]byte(ro), r); err != nil {
		overlogging.FromContext(ctx).Warnw("Error deserializing Rollout: "+ro,
			zap.Error(err))
		return nil
	}
	if !r.Validate() {
		overlogging.FromContext(ctx).Warnw("Deserializing Rollout is invalid: " + ro)
		return nil
	}
	return r
}
...
func (c *Reconciler) reconcileRollout(
	...
	cfg := overconfig.FromContext(ctx)
	...
	logger := overlogging.FromContext(ctx).Desugar().With(
