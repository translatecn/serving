...
func (cfg *Config) targetToStatus(ctx context.Context, r *v1.Route, tt *RevisionTarget,
	...
	for i := range revs {
		...
		result := v1.TrafficTarget{
			...
			Percent:        overptr.Int64(int64(rr.Percent)),
...
func mergeIfNecessary(rts RevisionTargets, rt RevisionTarget) RevisionTargets {
	for i := range rts {
		if rts[i].Tag == rt.Tag && rts[i].RevisionName == rt.RevisionName && *rt.LatestRevision == *rts[i].LatestRevision {
			rts[i].Percent = overptr.Int64(*rts[i].Percent + *rt.Percent)
			return rts
		}
	}
...
func consolidate(targets RevisionTargets) RevisionTargets {
	...
	for _, tt := range targets {
		...
		cur.TrafficTarget.Percent = overptr.Int64(*cur.TrafficTarget.Percent + *tt.TrafficTarget.Percent)
		byName[name] = cur
	}
	...
	if len(consolidated) == 1 {
		consolidated[0].TrafficTarget.Percent = overptr.Int64(100)
	}
	return consolidated
}
