...
func MakeClusterLocalCertificate(route *v1.Route, tag string, domains sets.Set[string], certClass string) *networkingv1alpha1.Certificate {
	...
	return &networkingv1alpha1.Certificate{
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{*overkmeta.NewControllerRef(route)},
			Annotations: overkmap.Filter(overkmap.Union(map[string]string{
		...
		Spec: networkingv1alpha1.CertificateSpec{
			...
			Domain:     "svc." + overnetwork.GetClusterDomainName(),
			SecretName: certName,
		},
	}
}
...
func MakeCertificate(owner overkmeta.OwnerRefableAccessor, ownerLabelKey string, dnsName string, certName string, certClass string, baseDomain string) *networkingv1alpha1.Certificate {
	return &networkingv1alpha1.Certificate{
		ObjectMeta: metav1.ObjectMeta{
			Name:            certName,
			Namespace:       owner.GetNamespace(),
			OwnerReferences: []metav1.OwnerReference{*overkmeta.NewControllerRef(owner)},
