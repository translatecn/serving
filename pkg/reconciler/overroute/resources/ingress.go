...
import (
	...
	"knative.dev/serving/pkg/reconciler/overroute/resources/names"
...
func MakeIngressWithRollout(
	...
	return &netv1alpha1.Ingress{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: overkmeta.UnionMaps(r.Labels, map[string]string{
				serving.RouteLabelKey:          r.Name,
				serving.RouteNamespaceLabelKey: r.Namespace,
			}),
			Annotations: overkmeta.FilterMap(overkmeta.UnionMaps(map[string]string{
				networking.IngressClassAnnotationKey: ingressClass,
				networking.RolloutAnnotationKey:      serializeRollout(ctx, ro),
			}, r.GetAnnotations()), ExcludedAnnotations.Has),
			OwnerReferences: []metav1.OwnerReference{*overkmeta.NewControllerRef(r)},
		},
		Spec: spec,
	}, nil
}
...
func serializeRollout(ctx context.Context, r *traffic.Rollout) string {
	...
	if err != nil {
		...
		overlogging.FromContext(ctx).Warnw("Error serializing Rollout: "+spew.Sprint(r),
			zap.Error(err))
		return ""
	}
	return string(sr)
...
func makeIngressSpec(
	...
	featuresConfig := overconfig.FromContextOrDefaults(ctx).Features
	networkConfig := overconfig.FromContextOrDefaults(ctx).Network
	...
	httpOption, err := servingnetworking.GetHTTPOption(ctx, overconfig.FromContext(ctx).Network, r.GetAnnotations())
	if err != nil {
		return netv1alpha1.IngressSpec{}, err
	}
